...
func processRetryControl(store beads.Store, bead beads.Bead, opts ProcessOptions) (ControlResult, error) {
	...
	attemptLog, err := appendAttemptLogValue(bead.Metadata[beadmeta.AttemptLogMetadataKey], attemptNum, result.Outcome, result.Reason, opts.tracef)
	if err != nil {
...
func processRalphControl(store beads.Store, bead beads.Bead, opts ProcessOptions) (ControlResult, error) {
	...
	attemptLog, err := appendAttemptLogValue(bead.Metadata[beadmeta.AttemptLogMetadataKey], iterationNum, checkResult.Outcome, checkResult.Stderr, opts.tracef)
	if err != nil {
		return ControlResult{}, fmt.Errorf("%s: recording attempt log: %w", bead.ID, err)
	}
